...
> AI: Use Bulk API 2.0 (not Bulk API 1.0) for large data volumes. Generate the full job lifecycle — create, upload CSV, close, poll, fetch results. Use the current version path (v68.0). For limit numbers, see limits/governor-limits.md.
> Release: Winter '27 | API: v68.0 | Updated: 2026-09
> Source: Verified against the Salesforce Bulk API 2.0 and Bulk API Developer Guide ("Version 68.0, Winter '27"). Endpoints, job states, CSV requirements, and the 150 MB / 150,000,000 record allocations confirmed in the guide. Limit numbers live in limits/governor-limits.md.
...
```
1. POST /services/data/v68.0/jobs/ingest
   { "object": "Contact", "operation": "insert", "contentType": "CSV" }
   -> response: job id, state "Open", contentUrl

2. PUT  /services/data/v68.0/jobs/ingest/{jobId}/batches
   Content-Type: text/csv
   <CSV data>

3. PATCH /services/data/v68.0/jobs/ingest/{jobId}
   { "state": "UploadComplete" }

4. Poll GET /services/data/v68.0/jobs/ingest/{jobId}
...
```
1. POST /services/data/v68.0/jobs/query
   { "operation": "query", "query": "SELECT Id, Name FROM Account" }
   // use "queryAll" to include deleted/archived rows

2. Poll GET /services/data/v68.0/jobs/query/{jobId} until state JobComplete

3. GET /services/data/v68.0/jobs/query/{jobId}/results
